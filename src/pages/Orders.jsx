...
function Orders(){
 ...
 return (
    <>{
      isAdmin ? (
        ...
        <div className={style.titleAndSearch}>
          ...
          <h1>{searchValue ? `Поиск по запросу: ` : "Все заказы"}
          </h1>
  ...
  {isLoading ? [...new Array(10)].map(()=><Skeleton/>) : (allOrders.length > 0 ? (
     ...
     allOrders.filter((item) => item._id.toLowerCase().includes(searchValue.toLowerCase())).map((item,index) =><Order item = {item}  orderId = {0} date = {date[index]} totalPrice = {0}></Order>)
